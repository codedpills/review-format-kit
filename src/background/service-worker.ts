...
async function performSync(): Promise<void> {
  try {
    ...
    if (result.success) {
      ...
      await showNotification('Sync Failed', result.error || 'Unknown error occurred');
    }
  } catch (error) {
    console.error('Error performing sync:', error);
    await showNotification('Sync Error', error instanceof Error ? error.message : 'Unknown error');
  }
}
