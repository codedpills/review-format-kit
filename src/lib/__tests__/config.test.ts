...
describe('ConfigManager', () => {
    ...
    describe('updateGroup', () => {
        ...
        it('should throw error for non-existent group', async () => {
            await expect(
                configManager.updateGroup('non-existent', { name: 'Test' })
    ...
    describe('deleteGroup', () => {
        ...
        it('should throw error when deleting last group', async () => {
            ...
            await expect(
                configManager.deleteGroup(secondGroupId)
            ).rejects.toThrow('Cannot delete the last');
        });
        ...
        it('should set new default if deleted group was default', async () => {
            ...
            expect(config.groups.length, 'Should have at least 2 groups from defaults').toBeGreaterThanOrEqual(2);
            ...
            if (!defaultGroup) {
                const groupStates = config.groups.map(g => `${g.id}: isDefault=${g.isDefault}`).join(', ');
            ...
            expect(newDefaultGroup, 'Should have a new default group after deleting the old default').toBeDefined();
    ...
    describe('addConvention', () => {
        ...
        it('should throw error for duplicate convention ID', async () => {
            ...
            await expect(
                configManager.addConvention(groupId, existingConvention)
    ...
    describe('reorderConventions', () => {
        it('should reorder conventions', async () => {
            ...
            const reorderedIds = [conventions[2].id, conventions[0].id, conventions[1].id, ...conventions.slice(3).map((c) => c.id)];
        ...
        it('should throw error for mismatched IDs', async () => {
            ...
            await expect(
                configManager.reorderConventions(groupId, ['id1', 'id2'])
