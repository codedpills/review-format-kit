...
function setupEventListeners(): void {
  ...
  document
    .getElementById('add-convention-btn')
    ?.addEventListener('click', () => openConventionModal());
...
function createGroupCard(group: ConventionGroup): HTMLElement {
  ...
  path.setAttribute(
    'd',
    'M4.427 7.427l3.396 3.396 3.396-3.396.781.781-4.177 4.177-4.177-4.177.781-.781z'
  );
  ...
  expandBtn.addEventListener('click', (e) => {
    ...
    expandBtn.querySelector('.expand-label')!.textContent = isExpanded
      ? 'Hide conventions'
      : 'Show conventions';
...
function renderConventionsList(): void {
  ...
  currentGroupConventions.forEach((convention, index) => {
    ...
    template.textContent =
      convention.template.substring(0, 50) + (convention.template.length > 50 ? '...' : '');
...
function saveConvention(): void {
  ...
  const convention: Convention = {
    id:
      editingConventionIndex !== null
        ? currentGroupConventions[editingConventionIndex].id
        : crypto.randomUUID(),
...
async function changeKeyboardShortcut(): Promise<void> {
  ...
  const newShortcut = prompt(
    'Enter new keyboard shortcut (e.g., Cmd+Shift+/):',
    currentConfig.settings.keyboardShortcut
  );
